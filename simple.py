...
def solve(board, turn, obstacle_num):
    return ((evaluate(c[0]), c[0], c[1]) for c in next_boards(board, turn, obstacle_num))
...
def process_turn():
    ...
    _ = [[int(i) for i in input().split()] for _ in range(HEIGHT)]
    _ = input()

    ob = max(obstacle_num - enemy_obstacle_num, 0)
    res = max(solve(board, turn, ob))
